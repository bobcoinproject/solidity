...
.. warning::
    The low-level ``call``, ``delegatecall`` and ``staticcall`` will return ``true`` in their first return value if the called account is non-existent, as part of the design of EVM. Existence must be checked prior to calling if desired.
